...
export async function GET(request: Request) {
  try {
    ...
    // Cross-reference each raffle entry with OSH inbound pilots.
    // Batch in parallel — the list is small during event week.
    const enrichedEntries = await Promise.all(
      entries.map(async (e) => {
        const userId = await lookupUserIdByEmail(e.email as string, supabaseUrl, serviceKey)
        return {
          ...e,
          flew_to_osh: userId !== null ? oshUserIds.has(userId) : null,
        }
