...
// Invite page is on the LANDING site, not the main app
...
async function sendInviteEmail(email: string, inviteToken: string): Promise<boolean> {
  ...
  if (!RESEND_API_KEY) {
    console.log(`[Waitlist Invite] Would send email to ${email} with invite URL: ${inviteUrl}`)
  ...
  try {
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: FROM_EMAIL,
        to: [email],
        subject: "✈️ You're in — PlaneWX is ready for you",
        html: emailHtml,
      }),
    })
    ...
    if (!response.ok) {
      ...
      console.error(`[Waitlist Invite] Resend error for ${email}:`, errorData)
      return false
    }

    const result = await response.json()
    console.log(`[Waitlist Invite] Email sent to ${email}, id: ${result.id}`)
    return true
  } catch (error) {
    console.error(`[Waitlist Invite] Failed to send email to ${email}:`, error)
    return false
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    if (fetchError) {
      console.error("[Waitlist Invite] Fetch error:", fetchError)
      return NextResponse.json(
    ...
    for (const entry of entries) {
      ...
      if (entry.status === 'invited' || entry.status === 'joined') {
        results.alreadyInvited.push(entry.email)
        continue
      }
      ...
      // Send email
      const emailSent = await sendInviteEmail(entry.email, inviteToken)
      ...
      if (emailSent) {
        ...
        const { error: updateError } = await supabase
          ...
          .update({
            ...
            approval_token: inviteToken,
            approval_token_expires_at: expiresAt.toISOString(),
          })
          .eq("id", entry.id)

        if (updateError) {
          console.error(`[Waitlist Invite] Update error for ${entry.email}:`, updateError)
          results.failed.push(entry.email)
        } else {
    ...
    return NextResponse.json({
      message: `Invited ${results.success.length} users`,
      results,
    })
  } catch (error) {
    console.error("[Waitlist Invite] Unexpected error:", error)
    return NextResponse.json(
